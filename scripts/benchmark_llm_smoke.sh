...
extract_real_seconds() {
  local logfile=$1
  awk '/^real / { print $2 }' "$logfile" | tail -n 1
}
...
for model_path in "${models[@]}"; do
  ...
  run_case "$model_path" "backend_load_and_context_probe" "$backend_log" || true
  run_case "$model_path" "reacts_to_javascript_backend_changes" "$js_log" || true
  run_case "$model_path" "reacts_to_typescript_frontend_changes" "$ts_log" || true
  ...
  {
    printf '%s\t' "$reaction_score"
    printf '%s\t' "$total_score"
    printf '%s\t' "$total_seconds"
    printf '%s\t' "$rel_model"
    printf '%s\t' "${backend_result:-n/a}"
    printf '%s\t' "${backend_seconds:-n/a}"
    printf '%s\t' "${js_result:-n/a}"
    printf '%s\t' "${js_status:-n/a}"
    printf '%s\t' "${js_seconds:-n/a}"
    printf '%s\t' "${js_detail:-}"
    printf '%s\t' "${ts_result:-n/a}"
    printf '%s\t' "${ts_status:-n/a}"
    printf '%s\t' "${ts_seconds:-n/a}"
    printf '%s\t' "${ts_detail:-}"
    printf '%s\n' "$model_log_dir"
  } >>"$results_tsv"
done
...
sort -t $'\t' -k1,1nr -k2,2nr -k3,3g -k4,4 "$results_tsv" >"$sorted_tsv"

csv_file="$output_dir/summary.csv"
md_file="$output_dir/summary.md"

{
  printf 'model_path,reaction_score,total_score,total_seconds,backend_result,backend_seconds,javascript_result,javascript_status,javascript_seconds,javascript_detail,typescript_result,typescript_status,typescript_seconds,typescript_detail,log_dir\n'
  while IFS=$'\t' read -r reaction_score total_score total_seconds model_path backend_result backend_seconds js_result js_status js_seconds js_detail ts_result ts_status ts_seconds ts_detail log_dir; do
...

{
  ...
  printf 'Command shape: one ignored smoke test per model, sequentially, with `--test-threads=1` for repeatability.\n\n'
  printf '| Rank | Model | Reaction Score | Total Score | Load | Load s | JS | JS status | JS s | TS | TS status | TS s | Total s |\n'
  printf '| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n'
  rank=1
  while IFS=$'\t' read -r reaction_score total_score total_seconds model_path backend_result backend_seconds js_result js_status js_seconds js_detail ts_result ts_status ts_seconds ts_detail log_dir; do
    printf '| %s | `%s` | %s/2 | %s/3 | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n' \
      "$rank" \
      "$model_path" \
      "$reaction_score" \
      "$total_score" \
    ...
    if [[ -n "$js_detail" || -n "$ts_detail" ]]; then
      printf '|  | Notes |  |  |  |  |  | `%s` |  |  | `%s` |  | Logs: `%s` |\n' \
        "${js_detail:-}" \
        "${ts_detail:-}" \
        "$log_dir"
    else
