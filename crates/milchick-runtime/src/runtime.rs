...
use crate::executor::{ExecutionReport, NotificationSink, ReviewConnector};
        ...
        for sink in &self.notification_sinks {
            for notification in notifications {
                let report = if strategy == ExecutionStrategy::DryRun {
                    milchick_core::model::NotificationDeliveryReport {
                        sink: sink.kind(),
                        delivered: false,
                        destination: None,
                    ...
                    match sink.send(notification).await {
                        Ok(report) => report,
                        Err(err) => milchick_core::model::NotificationDeliveryReport {
                            sink: sink.kind(),
                            delivered: false,
                            destination: None,
...
fn dry_run_review_report(actions: &[ReviewAction]) -> milchick_core::model::ReviewActionReport {
    let mut report = milchick_core::model::ReviewActionReport::default();

    for action in actions {
        report
            .applied
            .push(milchick_core::model::AppliedReviewAction {
                action: action.kind(),
                detail: Some("dry-run".to_string()),
            });
    }
