...
use crate::domain::codeowners::matcher::collect_section_requirements_for_snapshot;
...
pub fn plan_codeowners_assignments(
    ...
    let author = snapshot.details.author_username.as_str();
    let current_reviewers = &snapshot.details.reviewer_usernames;
...
mod tests {
    ...
    use crate::domain::codeowners::parser::parse_codeowners_str;
    use crate::gitlab::api::{
        ChangedFile, MergeRequestDetails, MergeRequestSnapshot, MergeRequestState,
    };

    fn snapshot(paths: &[&str], author: &str, reviewers: &[&str]) -> MergeRequestSnapshot {
